...
function printStack(currentState, stack) {
  var ret = '';
  for (let s of stack) {
    ret = ret + s.stack + ' (' + s.added + ')\n\t';
  }
  console.log('printStack: ' + currentState + '\nSTACK:\n\t' + ret);
  return currentState;
}

function emptyStack(currentState, stack) { // the last item in this._push array is on the top of stack
  if (stack.length === 0) {
    return this._next;
  } else {
    return 'ERROR';
...
function pushNext(currentState, stack) { // the last item in this._push array is on the top of stack
  if (this._push.constructor === Array) {
    for (let i of this._push) {
      stack.unshift({stack: i, added: currentState});
    }
  } else {
...
function reset(currentState, stack) { // the last item in this._push array is on the top of stack
  stack = [];
  return 'start';
}

function remPushNext(state, stack) {
  for (let i of this._rem) {
    if (stack.length > 1 && stack[0].stack === i) {
      stack.shift();
    } else {
      return 'ERROR';
    }
  ...
  if (this._push.constructor === Array) {
    for (let i of this._push) {
      stack.unshift({stack: i, added: state});
    }
  } else {
...
function pop(currentState, stack) {
  if (this._log) {
    printStack(currentState, stack);
  }
  var s = stack.shift();
  if (currentState === 'conditions' && !s) {
    return 'start';
  } //jump to start without semicolon at the end
  return s ? s.stack : 'ERROR';
}

function log(value, currentState, stack) {
  return [
    {type: 'ERROR1.' + currentState, value: '@'}
  ];
}

function detectVariables(value, currentSate, stack) {
...
const NameChar = '[-._:' + Letter + Digit + CombiningChar + Extender + ']';
const Name = '(?:[_:' + Letter + ']' + NameChar + '*)';
const XMLNAME = Name;
const TYPE_PREFIX = '(?:' + Name + '\\s*\\?)';

const R_SELECTOR_NAME = '(?:\\$[a-zA-Z_][a-zA-Z0-9_]*\\b)';
const R_VAR_OR_SELF = '(?:' + R_SELECTOR_NAME + '|\\$\\$)';
const R_VAR_REF = '(?:\\$[0-9]+)';

const R_STRING = "(?:'(?:[^'\\\\]|\\\\.)*'|\"(?:[^\"\\\\]|\\\\.)*\")";
const R_NUMBER = '(?:-?[0-9]+(?:\\.[0-9]+)?)';
const R_LITERAL = '(?:' + R_STRING + '|' + R_NUMBER + ')';

const R_OP = '(?:[-+*&]|\\bdiv\\b|\\bmod\\b)';
const R_LOGIC = '(?:\\band\\b|\\bor\\b|!)';
const R_CMP = '(?:\\?|!?=|!?~\\*?|[<>]=?|!?~)';
const R_OP_OTHER = '(?::=|,|(?:!\\s*)?\\bin\\b|->|::|\\.\\.|\\.|;)';
const R_OPERATOR = '(?:' + R_OP + '|' + R_LOGIC + '|' + R_CMP + '|' + R_OP_OTHER + ')';
const R_OCCURR = '(?:[0-9][0-9]*|[0-9][0-9]*[+-]?|[0-9][0-9]*\\.\\.[0-9][0-9]*)(?:\\|' +
...
var PmltqHighlightRules = function () {
  ...
  this.addNewRule = function (rules, tok, reg) {
    ...
    for (let r of rules) {
      this.$rules[r].push({token: tok, regex: reg});
    }
  };
  this.addAttribute = function (tok, a) {
  ...
  this.addUnknownToken = function (rules) {
    ...
    for (let r of rules) {
      this.$rules[r].push({token: 'UNKNOWN', regex: '.'});
    }
  };
  this.setDefaultSpaceToken = function (rules) {
    ...
    for (let r of rules) {
      this.$rules[r].push({token: 'SP', regex: '\\s*', next: r});
    }
  };
  ...
  this.addDefaultPopRule = function (rules) {
    ...
    for (let r of rules) {
      this.$rules[r].push({token: 'SP', regex: '\\s*', next: pop});
    }
  };
...
module.exports = PmltqHighlightRules;
